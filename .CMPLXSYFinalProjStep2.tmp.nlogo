...
to update-firefighters
  ask firefighters [
    ;; ------------------- DECISION PHASE -------------------
    ;; find the nearest burning patch
    let target min-one-of patches with [tree-state = "burning"] [distance myself]

    ;; if a fire still exists, move toward it
    if target != nobody [
      face target
      fd 1                           ;; move one patch per tick
    ]

    ;; -------------------- ACTION PHASE --------------------
    ;; if standing on a burning tree, suppress it
    ask patch-here [
      if tree-state = "burning" [
        set fire-intensity fire-intensity - extinguish-rate
        ;; visual cue: fade toward blue-grey as intensity drops
        set pcolor scale-color blue fire-intensity 0 max-burn-counter
        ;; if fully out, mark as extinguished and kill fire turtle
        if fire-intensity <= 0 [
